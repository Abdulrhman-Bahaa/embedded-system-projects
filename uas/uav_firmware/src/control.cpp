...
PidController::PidController(float kp, float ki, float kd, float ka, float sp, float min_roll_actuator_command,
                             float max_roll_actuator_command)
    : kp(kp), ki(ki), kd(kd), ka(ka), sp(sp), min_roll_actuator_command(min_roll_actuator_command),
      max_roll_actuator_command(max_roll_actuator_command), raw_pid_output(0), actual_pid_output(0), saturated(0),
      accumulated_error(0) {}

void
PidController::control(float pv) {
    current_time = millis();
